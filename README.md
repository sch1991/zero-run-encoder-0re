...
- 주제와는 별개로 이 프로젝트의 목적은 개인의 C언어 연습입니다.
- '최대한 많은 시스템에서 컴파일 가능하게 하자' 즉 크로스 플랫폼 지원을 첫 번째 목표로 하였습니다.
  - Window/Linux에서 모두 컴파일 가능하게 하기 위해 C 표준 라이브러리 위주로 사용하였고, 이 외에 MSVC/POSIX 차이가 있는 함수의 경우 매크로 전처리를 통해 환경에 따라 동일한 기능의 함수로 포팅되게 작성하였습니다.
  - Window/Linux에 따라 파일 경로 구분자['\\', '/']가 선택되게 작성하였습니다.
  - 32bit/64bit 환경에 따라 최적화된 자료형, 함수가 선택되게 작성하였습니다. (e.g., 필요한 경우에만 long long int 사용, size_t, ssize_t를 적극 활용, 시스템에 따라 32bit/64bit builtin 함수 선택)
  - 연합체(Union)를 활용하여 데이터를 처리할 때, 빅 엔디언/리틀 엔디언 여부를 고려하여 처리되도록 작성하였습니다.
- '비정상 종료 상황에서도 자원을 정리하고 종료하게 하자' 즉 우아한 종료 구현을 두 번째 목표로 하였습니다.
  - 4개의 스택을 구현하여 메모리(동적 할당), 파일 포인터, 임시 파일, 스레드를 관리하였고 Cleanup 함수를 구현하여 종료 시 스택의 모든 자원 및 파일을 정리하도록 작성하였습니다.
  - POSIX 표준의 Signal 함수에 Cleanup 함수를 콜백으로 등록하여 SIGINT/SIGSEGV/SIGTERM 신호 발생 시에도 우아한 종료가 실행되도록 구현하였습니다.
- 시스템 지연을 최소화하기 위해 inline, 정적 테이블 생성, 멀티스레딩을 활용 했습니다.
- 최적화를 위해 곱셈, 나눗셈, 소수점 연산 활용을 최소화하고 비트 연산, 비트 쉬프트 연산을 적극 활용하였습니다.
...
- 각 청크의 2-bit 패턴 [00, 01, 10, 11] 이후에 오는 최다 빈도 2-bit 패턴들을 기록한 '1Byte(=8bits) 청크 맵' 데이터를 복호화의 키 데이터로서 결과 파일에 저장합니다.
- 결과 파일의 크기를 줄이기 위해 청크 맵의 변화를 기록한 플래그 비트를 함께 저장합니다. (이전 청크 맵과 다른 청크 맵을 저장할 경우 플래그를 1로, 새로운 청크 맵이 이전과 같은 경우 플래그를 0으로 저장하고 청크 맵을 추가로 저장하지 않습니다.)
...
### - 분석
- 샘플 파일 압축 결과, ZIP 포맷의 압축 알고리즘은 13\~74% 수준의 압축률을 보인 반면, 본 프로젝트의 압축 알고리즘은 0\~8% 압축률에 그쳤으며, 일부 파일에서 마이너스 압축률(압축 후 파일 크기가 증가)을 보였다.
- 본 프로젝트의 알고리즘은 정보 엔트로피가 낮은 샘플 파일을 입력했음에도 불구하고 유의미한 압축률을 보이지 못했기에 압축 알고리즘으로서의 실용성은 부족한 것으로 판단된다.
